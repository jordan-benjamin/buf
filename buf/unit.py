...
class UnitLadder:
    ...
    def can_scale_down(self, symbol):
        if symbol not in self.symbol_to_info:
            print(f"Unit symbol not found: '{symbol}' is not in unit ladder.")
            exit()
    ...
    # TODO: are errors the right things to be raising here? oppsed to print + exit?
    def scale_up(self, symbol):
        if symbol not in self.symbol_to_info:
            print(f"Unit symbol not found: '{symbol}' is not in unit ladder.")
            exit()
        ...
        if unit_info.greater:
            return unit_info.greater.symbols[0], (unit_info.greater.scale_factor / unit_info.scale_factor)
        else:
            print(f"No greater unit: '{symbol}' does not have a unit greater than it in the ladder.")
            exit()


    def scale_down(self, symbol):
        if symbol not in self.symbol_to_info:
            print(f"Unit symbol not found: '{symbol}' is not in unit ladder.")
            exit()
        ...
        if unit_info.lesser:
            return unit_info.lesser.symbols[0], (unit_info.lesser.scale_factor / unit_info.scale_factor)
        else:
...
mass_units = UnitLadder({"kg" : 1000, "g" : 1, "mg" : 1e-3, "ug": 1e-6, "µg" : 1e-6})

# Standardised to molar.
concentration_units = UnitLadder({"M" : 1, "mM" : 1e-3, "uM" : 1e-6, "µM" : 1e-6})
